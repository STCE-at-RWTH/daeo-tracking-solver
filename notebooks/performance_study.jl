...
md"""
It's important to note that the cost of finding all local optima and their optimizers is similar to the cost of only finding the global optimum, because the value, gradient, and Hessian of ``h(t,x,y; p)`` are computed simultaneously via a combined driver. The Hessian is small (``y∈\mathbb R`` or ``y∈\mathbb R^2``), so we don't lose *too* much here.

If the cost of optimization drives the overall computation cost of the DAEO solver, there's an easy flaw to fix to save CPU cycles. 
"""
