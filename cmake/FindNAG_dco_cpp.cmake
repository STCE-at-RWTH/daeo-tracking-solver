...
function(_dco_get_installation_path prodcode installation_path)
  if(WIN32)
    ...
    string(CONCAT _dco_reg_path "HKLM\\SOFTWARE\\WOW6432Node\\Microsoft\\"
                  "Windows\\CurrentVersion\\Uninstall")
    execute_process(
      COMMAND "reg" "query" "${_dco_reg_path}\\${_dco_appid}"
      OUTPUT_VARIABLE _installation_path_loc
      ERROR_QUIET)
    if(_installation_path_loc)
      string(REGEX MATCH "InstallLocation[ ]*REG_SZ[ ]*([^\n]*)"
                   _installation_path_loc ${_installation_path_loc})
      string(REGEX REPLACE "InstallLocation[ ]*REG_SZ[ ]*" ""
                           _installation_path_loc ${_installation_path_loc})
    endif()
    ...
    set(_dco_def_dirs $ENV{HOME}/NAG/${prodcode} /opt/NAG/${prodcode}
                      /opt/${prodcode})
    find_file(
  ...
  if(NAG_dco_cpp_INCLUDE_DIR)
    ...
    if(WIN32)
      ...
      foreach(_dco_version_major 9;8;7;6;5;4;3;2)
        if(NAG_dco_cpp_DIR)
          message(${NAG_dco_cpp_DIR})
          break()
        endif()
        foreach(_dco_version_minor 9;8;7;6;5;4;3;2;1;0)
          # Try to find non-license-managed version
          _dco_get_installation_path(
            "dclin${_dco_version_major}${_dco_version_minor}nn" NAG_dco_cpp_DIR)
          if(NAG_dco_cpp_DIR)
            break()
          endif()
...
find_path(NAG_dco_cpp_INCLUDE_DIR dco.hpp
          HINTS ${NAG_dco_cpp_DIR}/include ${NAG_dco_cpp_DIR}/src
                ${NAG_dco_cpp_DIR})
...
string(REGEX REPLACE ".*used DCO_FLAGS: \(.*\)" "\\1" _DCO_HPP_BUILD_DEFINES
                     ${FILE_CONTENTS})
...
if(WIN32)
  ...
  find_library(
    NAG_dco_cpp_LIBRARY_MD
    NAMES ${_dco_library_name}_MD
    HINTS ${NAG_dco_cpp_DIR}/lib)
  find_library(
    NAG_dco_cpp_LIBRARY_MT
    NAMES ${_dco_library_name}_MT
    HINTS ${NAG_dco_cpp_DIR}/lib)
  find_library(
    NAG_dco_cpp_LIBRARY_MDd
    NAMES ${_dco_library_name}_MDd
    HINTS ${NAG_dco_cpp_DIR}/lib)
  find_library(
    NAG_dco_cpp_LIBRARY_MTd
    NAMES ${_dco_library_name}_MTd
    HINTS ${NAG_dco_cpp_DIR}/lib)
  ...
  if(NAG_dco_cpp_LIBRARY)
    if(WIN32)
      ...
      set_target_properties(
        NAG::dco_cpp_MT PROPERTIES IMPORTED_LOCATION
                                   "${NAG_dco_cpp_LIBRARY_MT}")
      set_target_properties(
        NAG::dco_cpp_MT PROPERTIES INTERFACE_LINK_OPTIONS
                                   "${_dco_cpp_compile_options}")
      add_library(NAG::dco_cpp_MTd STATIC IMPORTED)
      set_target_properties(
        NAG::dco_cpp_MTd PROPERTIES IMPORTED_LOCATION
                                    "${NAG_dco_cpp_LIBRARY_MTd}")
      set_target_properties(
        NAG::dco_cpp_MTd PROPERTIES INTERFACE_LINK_OPTIONS
                                    "${_dco_cpp_compile_options}")
      add_library(NAG::dco_cpp_MD STATIC IMPORTED)
      set_target_properties(
        NAG::dco_cpp_MD PROPERTIES IMPORTED_LOCATION
                                   "${NAG_dco_cpp_LIBRARY_MD}")
      set_target_properties(
        NAG::dco_cpp_MD PROPERTIES INTERFACE_LINK_OPTIONS
                                   "${_dco_cpp_compile_options}")
      add_library(NAG::dco_cpp_MDd STATIC IMPORTED)
      set_target_properties(
        NAG::dco_cpp_MDd PROPERTIES IMPORTED_LOCATION
                                    "${NAG_dco_cpp_LIBRARY_MDd}")
      set_target_properties(
        NAG::dco_cpp_MDd PROPERTIES INTERFACE_LINK_OPTIONS
                                    "${_dco_cpp_compile_options}")
      ...
      if((CMAKE_MSVC_RUNTIME_LIBRARY MATCHES "DLL") OR (NAG_USE_DYNAMIC_RUNTIME
        ...
        set_target_properties(
          NAG::dco_cpp PROPERTIES INTERFACE_LINK_LIBRARIES
      ...
      set_property(
        TARGET NAG::dco_cpp
        APPEND
        PROPERTY INTERFACE_LINK_LIBRARIES "user32" "advapi32" "ws2_32")
      set_target_properties(NAG::dco_cpp PROPERTIES INTERFACE_LINK_DIRECTORIES
                                                    "${_dco_library_dir}")
    else()
