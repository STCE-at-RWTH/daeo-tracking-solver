...
struct NumTraits<boost::numeric::interval<T, POLICIES>> : NumTraits<T> {
  ...
  enum {
    ...
    IsSigned = NumTraits<T>::IsSigned, // does this make sense? intervals of
                                       // uints or ufloats don't seem useful.
    RequireInitialization = 1, // intervals need initialization (probably?)
    ReadCost = 2, // could also be reasonably set to 1, even though there are
                  // two values
    AddCost = 2,  // two numbers
    MulCost = 2   // two numbers
  };

  // Intervals cannot be made out of complex numbers due to total ordering
  // requirement
  using Real = boost::numeric::interval<T, POLICIES>;
  using Literal = T;
  // If we end up doing lots of integer interval work, this should be
  // specialized
