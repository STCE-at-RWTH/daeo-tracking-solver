...
public:
  ...
  YT value(NUMERIC_T const t, XT const x, Eigen::Vector<YT, YDIMS> const &y,
           Eigen::Vector<NUMERIC_T, PDIMS> const &p) const {
    return m_fn(t, x, y, p);
  }

  template <typename NUMERIC_T, typename Y_ACTIVE_T, int YDIMS, int PDIMS>
  Eigen::Vector<Y_ACTIVE_T, YDIMS>
  grad_y(NUMERIC_T const t, NUMERIC_T const x,
  ...
  hess_y(NUMERIC_T const t, NUMERIC_T const x,
    ...
    for (int hrow = 0; hrow < y.rows(); hrow++) {
      ...
      dco::value(dco::derivative(h_active)) =
          1; // set sensitivity to wobbles in h to 1
      // dhdy[hrow] = dco::derivative(dco::value(h_active));   // wobble in h
      // generated by wiggling y[hrow]
      tape->interpret_adjoint_and_reset_to(
          start_position); // interpret and rewind the tape
      ...
      dco::derivative(dco::value(y_active(hrow))) =
          0; // no longer wiggling y[hrow]
    }
    return d2hdy2;
  }
