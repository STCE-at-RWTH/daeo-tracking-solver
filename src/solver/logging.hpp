...
constexpr char OPTIMIZER_LOG_COLUMNS[]{"TASKNUM\tTSTAMP\tEVENTID\tEXTRACODE\tX\tH\tDHDX\tD2HDX2\tCONVERGENCE"};
...
public:
    BNBOptimizerLogger(std::string const &tag)
        : m_threadcount{1}
    {
        outs.emplace_back(fmt::format("{}/{}_optimizer_thread_0.tsv", DATA_OUTPUT_DIR, tag));
        ...
        : m_threadcount{t_threads}
    {
        ...
        {
            outs.emplace_back(fmt::format("{}/{}_optimizer_thread_{}.tsv", DATA_OUTPUT_DIR, tag, i));
    ...
    void log_computation_begin(sys_time_point_t time, size_t tasknum, Y const &domain, size_t threadid = 0)
    {
    ...
    template <std::ranges::range Y>
    void log_task_begin(sys_time_point_t time, size_t tasknum, Y const &y, size_t threadid = 0)
    {
        fmt::print(outs[threadid], LOG_TNUM_TSTAMP, tasknum, time - m_logging_start);
    ...
    template <std::ranges::range Y>
    void log_task_complete(sys_time_point_t time, size_t tasknum, Y const &y, size_t reason, size_t threadid = 0)
    {
        fmt::print(outs[threadid], LOG_TNUM_TSTAMP, tasknum, time - m_logging_start);
    ...
    void log_gradient_test(sys_time_point_t time, size_t tasknum,
                           Y const &y, T const &h,
...
public:
    DAEOSolverLogger(std::string const &tag) : out(fmt::format("{}/{}_solver_log.tsv", DATA_OUTPUT_DIR, tag))
    {
    ...
    void log_computation_end(sys_time_point_t const tstamp, size_t const iter,
                             T const t, T const x, Y const &y, size_t const i_star)
    {
    ...
    void log_time_step(sys_time_point_t const tstamp, size_t const iter,
                       T const t, T const dt,
                       T const x, T const dx,
